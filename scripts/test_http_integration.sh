...
image="${FERRICSTORE_TEST_IMAGE:-quay.io/ferricstore/ferricstore:0.11.14@sha256:f7d29befefa15bce4b3755bf786cf7620c814f13bbd336c0d9955581b323b60e}"
container="ferricstore-elixir-http-integration-$$"
tls_dir="$(mktemp -d /tmp/ferricstore-elixir-http-integration.XXXXXX)"
username="sdk-http"
...
cleanup() {
  ...
  case "$tls_dir" in
    /tmp/ferricstore-elixir-http-integration.*) rm -rf "$tls_dir" ;;
  esac
}
trap cleanup EXIT

for command in docker openssl curl grep mix; do
...
for http2 in false true; do
  env FERRICSTORE_INTEGRATION=1 FERRICSTORE_TEST_URL="https://127.0.0.1:$port" FERRICSTORE_USERNAME="$username" FERRICSTORE_PASSWORD="$password" FERRICSTORE_CA_FILE="$tls_dir/ca.pem" FERRICSTORE_HTTP2="$http2" MIX_ENV=test mix test --include integration test/ferric_store/client_integration_test.exs
done
